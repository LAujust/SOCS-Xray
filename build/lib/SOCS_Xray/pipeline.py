...
class Pipeline(object):
    ...
    def run(self,dt=[-5,30],show_progress=True,wxt_radii:float=3.5,fxt_radii:float=20,update_result=False,fxt_search_max:float=3000):
        ...
        if len(self.ztf_match) > 0:
            ...
            ztf_match['pipeline'] = ['WXT-ZTF'] * len(ztf_match)
        ...
        if len(self.lsst_match) > 0:
            lsst_match = self.lsst_match
            lsst_match['fxt_name'] = ['None'] * len(lsst_match)
            lsst_match['pipeline'] = ['WXT-LSST'] * len(lsst_match)
            lsst_match.rename_columns(['simbad_name'],['ep_name'])
            lsst_match = lsst_match[col_names]
            for col,col_type in zip(col_names,col_types):
                lsst_match[col] = lsst_match[col].astype(col_type)
            res_table = vstack((res_table,lsst_match[col_names]))
        ...
        if len(self.fxt_lsst_match) > 0:
            fxt_lsst_match = self.fxt_lsst_match
            fxt_lsst_match['ep_link'] = ['https://ep.bao.ac.cn/ep/data_center/fxt_src_detail/%s'%id for id in fxt_lsst_match['id']]
            fxt_lsst_match['pipeline'] = ['FXT-LSST'] * len(fxt_lsst_match)
            fxt_lsst_match.rename_columns(['target_name'],['ep_name'])
            fxt_lsst_match = fxt_lsst_match[col_names]
            for col,col_type in zip(col_names,col_types):
                fxt_lsst_match[col] = fxt_lsst_match[col].astype(col_type)
            res_table = vstack((res_table,fxt_lsst_match[col_names]))
        ...
        LABELS = ['WXT-TNS','WXT-ZTF','WXT-ZTF','FXT-TNS','FXT-ZTF','FXT-LSST']
        html_parts = []
        for i,tbl in enumerate([self.tns_match,self.ztf_match,self.lsst_match,self.fxt_tns_match,self.fxt_ztf_match,self.fxt_lsst_match]):
            if len(tbl) > 0:
                df = tbl.to_pandas()
            ...
            if len(self.uniform_match) > 0:
                ...
                self.uniform_html = f"""
                                        ...
                                        <body>
                                            "<br>"{html_table}
                                            <br>
                                            <p>Matched with {len(self.TNS_table)} TNS sources, {len(self.ZTF_clean)} ZTF sources, {len(self.LSST_clean)} LSST sources.
    ...
    def update_ZTF(self,ndays=10):
        ...
        try:
            ...
            print(f"Alerce: ndet>1: {len(alerce_table_2)}, ndet=1: {len(alerce_table_1)}")
