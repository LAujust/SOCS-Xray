...
def get_Lasair(ndays,survey='ztf',lsst_ncut=5):
    ...
    if survey == 'ztf':
        ...
        API_TOKEN = '661d225f1502d5d2bb98eb16df21b53745acefb4'
        
        L = lasair(API_TOKEN, endpoint=endpoint)
        
        rows = L.query(
            """objects.diaObjectId as objectId,
            objects.ra AS ramean,
            objects.decl as decmean,
            objects.firstDiaSourceMjdTai AS mjdmin,
            objects.lastDiaSourceMjdTai AS mjdmax,
            objects.nDiaSources AS ndet,
            sherlock_classifications.classification""",
            """objects,
            sherlock_classifications,
            watchlist_hits""",
            """objects.diaObjectId = sherlock_classifications.diaObjectId
            AND objects.lastDiaSourceMjdTai > mjdnow() - %s
            AND objects.latest_psfFlux > 0
            AND objects.nSourcesGood > %s
            AND sherlock_classifications.classification IN ('SN','NT','ORPHAN')
            """ % (ndays,lsst_ncut),
            limit=10000
        )
        
    else:
        raise ValueError("Unsupported survey. Choose 'ztf' or 'lsst'.")
